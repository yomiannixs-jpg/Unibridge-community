...
import {
  AtSign, Bookmark, Compass, Hash, History, Home, LogIn, MessageSquare,
  Plus, Radio, Search, Settings, ShieldCheck, UserCircle, UserPlus, Users,
} from "lucide-react";
    ...
    <div className="min-h-[100dvh] bg-slate-50 text-slate-950">
      <header className="sticky top-0 z-40 border-b bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3">
          ...
          <Link href="/create" className="rounded-full bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-700">Create</Link>
          <Link href={isAuthenticated ? "/profile" : "/auth"} className="hidden items-center gap-2 rounded-full border bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50 md:flex">
            {isAuthenticated ? <span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-800 text-xs text-white">{user?.avatarInitials ?? "DS"}</span> : <LogIn className="h-4 w-4" />}
            {isAuthenticated ? user?.displayName ?? "Demo Student" : "Login"}
          </Link>
          <NotificationBell />
        </div>
      </header>
          ...
          <div className="sticky top-24 rounded-3xl border bg-white p-3 shadow-sm">
            <nav className="space-y-1">
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold transition ${active ? "bg-blue-50 text-blue-800" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`}>
          ...
          <div className="sticky top-24 space-y-4">
            ...
            <Link href="/moderation" className="block rounded-3xl border bg-white p-5 shadow-sm transition hover:border-blue-300 hover:shadow-md">
              ...
              <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-slate-600">
                <li>Be helpful and respectful.</li><li>No fake scholarships, spam, or paid-agent bait.</li><li>Use clear titles and include country/context.</li><li>Protect private information.</li>
              </ol>
            </Link>
          </div>
        </aside>
